...
f = open('authors.json', encoding="utf8")
...
# Load degrees.json
with open('degrees.json', encoding = 'utf-8') as inputfile:
  degrees = pd.read_json(inputfile)
degrees = degrees.drop('title', 0)
degrees = degrees.T
degrees = degrees.rename(columns = {'doi':'DOI'})
# Left join with dataframe on doi
df = pd.merge(df, degrees, how = "left", on =["DOI", "DOI"])
...
#check if uni_locations.json exists so that the API use is reduced
if os.path.isfile("uni_locations.json") and os.access("uni_locations.json", os.R_OK):
    f = open ('uni_locations.json', "r")
    uni_locations = json.loads(f.read())
    for university in uni_locations:
        best = 999999
        lat = uni_locations[university]['lat']
        lng = uni_locations[university]['lng']
        for city in cities:
...
#call opencagedata API to compare University locations to nearest city stats
else:
...
try:
    ...
    print('merged_data.tsv already exists')
